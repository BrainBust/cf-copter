...
(function ($) {
  var JqueryBracket = function (opts) {
    ...
    var topCon = $('<div class="jQBracket ' + opts.dir + '" style="margin: 0 auto;"></div>').appendTo(opts.el.empty())
    ...
    var Match = function (round, data, idx, results, renderCb) {
      function connector(height, shift, teamCon) {
        ...
        dst.css('width', width-10 + 'px');
        dst.css(align, -width+10 + 'px');
        if (drop)
          dst.css('bottom', '0px');
        else
          dst.css('top', '0px');
            ...
            if ((match.winner().name !== null && match.winner().name === losers.winner().name)) {
              ...
              var round = finals.addRound(function () {
                ...
                if (_isResized === false) {
                  if (rematch) {
                    _isResized = true
                    topCon.css('width', (parseInt(topCon.css('width'), 10) + 190) + 'px')
                  }
                }
    ...
    var height = data.teams.length * 70
    ...
    if (opts.save)
      topCon.css('width', rounds * 190 + 40)
    else
      topCon.css('width', rounds * 190 + 10)
